  ...
  template: `
    <div dir="rtl" style="padding-bottom:32px;">
      ...
      <div *ngIf="loading()" style="display:flex;flex-direction:column;gap:16px;">
        <div style="height:80px;border-radius:16px;background:var(--bg-card);"></div>
        <div style="height:100px;border-radius:16px;background:var(--bg-card);"></div>
        <div *ngFor="let i of [1,2]" style="height:180px;border-radius:16px;background:var(--bg-card);"></div>
      </div>

      <ng-container *ngIf="!loading() && summary()">

        <!-- Welcome banner — like student app welcome card -->
        <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:20px;padding:20px;margin-bottom:16px;position:relative;overflow:hidden;">
          <!-- Subtle glow -->
          ...
          <div style="position:relative;">
            <p style="color:var(--text-secondary);font-size:12px;margin:0 0 4px;">أهلاً بك 👋</p>
            <h1 style="color:var(--text-primary);font-size:20px;font-weight:800;margin:0 0 12px;">{{ summary().parentName || 'ولي الأمر' }}</h1>
            <!-- Progress bar: avg completion -->
            <div style="background:var(--bg-inset);border-radius:8px;padding:12px;">
              <div style="display:flex;justify-content:space-between;font-size:11px;margin-bottom:6px;">
                <span style="color:var(--text-secondary);font-weight:600;">متابعة الأبناء</span>
                <span style="color:#4BBBA0;font-weight:700;">{{ summary().childrenCount }} {{ summary().childrenCount === 1 ? 'ابن' : 'أبناء' }}</span>
              </div>
              <div style="height:6px;border-radius:3px;background:var(--bg-card-alt);overflow:hidden;">
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:20px;">
          <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:14px;padding:14px;text-align:center;">
            <div style="font-size:22px;font-weight:800;color:var(--text-primary);">{{ summary().childrenCount }}</div>
            <div style="color:var(--text-secondary);font-size:10px;margin-top:3px;font-weight:600;">الأبناء</div>
          </div>
          <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:14px;padding:14px;text-align:center;">
            <div style="font-size:22px;font-weight:800;color:#4BBBA0;">{{ totalEnrollments() }}</div>
            <div style="color:var(--text-secondary);font-size:10px;margin-top:3px;font-weight:600;">اشتراكات</div>
          </div>
          <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:14px;padding:14px;text-align:center;">
            <div style="font-size:22px;font-weight:800;color:#F59239;">{{ totalLessons() }}</div>
            <div style="color:var(--text-secondary);font-size:10px;margin-top:3px;font-weight:600;">دروس</div>
          </div>
        </div>

        <!-- Section label -->
        <p style="color:var(--text-secondary);font-size:11px;font-weight:700;letter-spacing:.6px;text-transform:uppercase;margin-bottom:12px;">أبنائي</p>
        ...
        <div style="display:flex;flex-direction:column;gap:14px;">
          <a *ngFor="let child of summary().children; let i = index"
             ...
             style="text-decoration:none;display:block;background:var(--bg-card);border:1px solid var(--border);border-radius:20px;overflow:hidden;transition:border-color 200ms,transform 150ms;"
            ...
            <div style="padding:16px 16px 10px;display:flex;align-items:center;gap:14px;">
              ...
              <div style="width:52px;height:52px;border-radius:14px;flex-shrink:0;overflow:hidden;display:flex;align-items:center;justify-content:center;color:var(--text-primary);font-weight:800;font-size:17px;position:relative;"
              ...
              <div style="flex:1;min-width:0;">
                <h3 style="color:var(--text-primary);font-weight:700;margin:0 0 5px;font-size:15px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                  {{ child.studentName }}
                </h3>
                <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
                  <span style="color:var(--text-secondary);font-size:12px;">{{ child.grade }}</span>
              ...
              <div *ngIf="child.unreadNotifications > 0"
                   style="width:20px;height:20px;border-radius:50%;background:#ef4444;color:var(--text-primary);font-size:10px;display:flex;align-items:center;justify-content:center;font-weight:800;flex-shrink:0;">
                {{ child.unreadNotifications }}
              </div>
            </div>

            <!-- Code + Phone — styled like student info chips -->
            <div style="padding:8px 16px;display:flex;gap:14px;align-items:center;background:var(--bg-inset);margin:0 12px;border-radius:10px;margin-bottom:10px;">
              <div style="display:flex;align-items:center;gap:5px;">
                <span class="material-icons-round" style="font-size:12px;color:var(--text-secondary);">badge</span>
                <span style="color:var(--text-secondary);font-size:11px;">كود:</span>
              ...
              <div *ngIf="child.phone" style="display:flex;align-items:center;gap:5px;">
                <span class="material-icons-round" style="font-size:12px;color:var(--text-secondary);">phone</span>
                <span style="color:var(--text-secondary);font-size:11px;font-family:monospace;direction:ltr;">{{ child.phone }}</span>
              </div>
            </div>
            ...
            <div style="padding:10px 16px 14px;display:grid;grid-template-columns:repeat(3,1fr);gap:4px;">
              <div style="text-align:center;">
                <div style="color:var(--text-primary);font-weight:800;font-size:17px;">{{ child.activeEnrollments }}</div>
                <div style="color:var(--text-secondary);font-size:10px;margin-top:2px;font-weight:600;">اشتراكات</div>
              </div>
              <div style="text-align:center;border-inline:1px solid var(--border);">
                ...
                <div style="color:var(--text-secondary);font-size:10px;margin-top:2px;font-weight:600;">دروس مكتملة</div>
              </div>
              <div style="text-align:center;">
                <div style="color:#F59239;font-weight:800;font-size:17px;">{{ child.totalAttendance }}</div>
                <div style="color:var(--text-secondary);font-size:10px;margin-top:2px;font-weight:600;">حضور</div>
              </div>
            </div>

            <!-- CTA footer — like student card "الدخول للكورس" button -->
            <div style="border-top:1px solid var(--border);padding:10px 16px;display:flex;justify-content:space-between;align-items:center;">
              <span style="color:var(--text-secondary);font-size:11px;">عرض ملف الابن</span>
        ...
        <div *ngIf="summary().children?.length === 0"
             style="text-align:center;padding:64px 0;color:var(--text-secondary);">
